...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomServiceHdle;                    /**< Service handle */
...
#define COPY_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: Service, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: Service \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARWRITE:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomServiceHdle, CustomContext.CustomCharwriteHdle, SizeCharwrite, pPayload);
